...
class BasicLayer(nn.Module):
    def __init__(self, downsample_scale, depth, channel, num_heads, window_size, grid_size, quant_size,
        ...
        self.window_size = window_size # 一个窗体的大小
...
class Stratified(nn.Module):
    ...
    def forward(self, feats, xyz, offset, batch, neighbor_idx):
        # feat zyzrgb
        # xyz
        feats_stack = [] # 为了记录feat的变化过程
        xyz_stack = [] # 为了记录xyz的变化过程
        offset_stack = [] # 为了记录offset的变化过程

        # 结构
        # point embeding -> transfromer block*2 + downsample -> blk*2 + downsample-> blk*6 + downsample-> blk*2
        # 他这里把downsample放到上面一层的结尾，然后最后一层没有downsample。
        # the first-layer point embedding module is used for local aggregation
        # Input point clouds firstly go through the Point Embedding module to aggregate local structure information
        # 给每个点加上局部信息， dgcnn是可以采用的，改进一下再。
